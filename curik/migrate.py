...
from .paths import content_dir as content_dir_fn, hugo_toml_path, theme_dir
...
def inventory_course(repo_path: str | Path) -> dict[str, Any]:
    ...
    if (root / "hugo.toml").is_file() or hugo_toml_path(root).is_file():
...
def migrate_structure(
    ...
    if not _content_dir.is_dir():
        ...
        created.append("site/content")

    # Copy the Hugo theme into site/themes/curriculum-hugo-theme/
    ...
    if theme_source.is_dir():
        ...
        created.append(f"site/themes/{THEME_NAME}")

    # Create site/hugo.toml
    hugo_path = hugo_toml_path(root)
    if not hugo_path.is_file():
        ...
        created.append("site/hugo.toml")

    # Create root _index.md
    root_index = _content_dir / "_index.md"
    if not root_index.is_file():
        title = root.name.replace("-", " ").replace("_", " ").title()
        root_index.write_text(f"# {title}\n\nCourse overview.\n", encoding="utf-8")
        created.append("site/content/_index.md")

    # Create module directories with _index.md stubs
    for mod_name in modules:
        mod_dir = _content_dir / mod_name
        if not mod_dir.is_dir():
            mod_dir.mkdir(parents=True, exist_ok=True)
            created.append(f"site/content/{mod_name}")

        index_path = mod_dir / "_index.md"
        if not index_path.is_file():
            title = mod_name.replace("-", " ").replace("_", " ").title()
            index_path.write_text(f"# {title}\n\nModule overview.\n", encoding="utf-8")
            created.append(f"site/content/{mod_name}/_index.md")

    return {"created": created}


# Paths that sequester_content() will never move.
# Includes everything `init_course` itself creates — sequester is meant to
# clear pre-existing repo content, not undo curik's own setup.
_PROTECTED_NAMES = frozenset({
    CURIK_DIR,    # .course/
    ".git",
    ".github",    # workflows/deploy-pages.yml installed by init
    ".gitignore", # installed by init
    ".mcp.json",
    ".claude",
    ".vscode",
    "CLAUDE.md",
